...
use crate::{
    Autoscroll, DefaultDiffHunkRenderer, DiffHunkRenderer, DiffReviewHandler, Editor, EditorEvent,
    EditorSettings, ToggleSoftWrap,
...
impl DiffHunkRenderer for SplitLhsDiffHunkRenderer {
    ...
    fn render_hunk_hollow(
        ...
        let renderer = splittable.read(cx).rhs_editor.read(cx).diff_hunk_renderer();
        renderer.render_hunk_hollow(status, buffer_id, cx)
    }
    ...
    fn render_diff_row_hollow(
        ...
        let splittable = self.splittable.upgrade()?;
        let renderer = splittable.read(cx).rhs_editor.read(cx).diff_hunk_renderer();
        renderer.render_diff_row_hollow(status, buffer_id, buffer_row, cx)
    }
}
